...
default; cli channel and any expensive-tier panelist need explicit the owner
approval; panelists blind; judge is a separate cold worker — never the
orchestrating session; anonymized judging; provenance under agent-os/state/fusion-runs/.
...
judge    : spawn as an independent Codex/Claude worker with ONLY the contract
           projection + packet + anonymized answers; per
           agent-os/adapters/codex-workflow.md, record a recoverable worker id
           and choose the cheapest capable independent judge tier.
