---
name: agentos
description: AgentOS 三省六部 chain on Codex — the user-invoked relay (太监) that carries the user's exact words to the 中书省 Desktop thread and the seat's reply back. Use ONLY when the user explicitly invokes it ($agentos, "三省六部", "走链"); never auto-trigger from a task that merely looks big.
---

# AgentOS — 三省六部 Relay (Codex)

Kernel: `agent-os/` — chain order and seat methods in
`agent-os/workflows/{zhongshu,menxia,shangshu,executor,yushi}.md`; Codex
transport in `agent-os/adapters/codex-workflow.md`. (Claude Code runs the same
kernel with a different transport: there the invoking session itself is 中书 —
see `.claude/skills/agentos/SKILL.md`.)

Invoking this skill turns THIS thread into the relay seat (`agentos-relay`)
...
1. New task: `python3 agent-os/tools/aos_task_record.py create --task t<YYYYMMDD-HHMM> --goal "<the user's exact words>"`.
   "Exact words" includes the invocation token: preserve the user's full
   `$agentos ...` message. Use that same full line in the 中书 prompt so the
...
3. Open or continue the 中书省 thread (below). Its first message begins
   `你是中书，任务 <id>` and carries the user's exact words; nothing else
   about what you think the task means.

## Every later user message

Record it: `append --task <id> --role relay --kind user_message --status ok --text "<exact words, newlines kept>"`.
Send the exact words to the 中书 thread with the task id. Wait for the reply and
...
writable and the relay's turn is never blocked.
