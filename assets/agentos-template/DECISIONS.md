...
### 2026-08-17 — The chain is opt-in through one relay skill; 中书 is a seat, not the default session (owner directive)
...
### 2026-08-17 — Codex seats use Desktop-native local threads with role-skill receipts (owner directive)
...
### 2026-08-16 — The chain order is enforced by hooks on mechanical facts; seats are runtime-native agents on both runtimes (owner directive)
...
- Evidence boundary: `pytest tests` 87 passed + 1028 subtests; `aos-lint.py` PASS; installer tests 6 passed. Claude live acceptance in a scratch copy: task `probe-note-file` ran user_message → menxia review+comparison/pass → shangshu dispatch → executor result → integration → delivery with the gate denying 6 premature calls (2 shangshu spawns before pass, 4 foreign-role ledger writes) and the file created by the executor. Codex: a `codex exec` probe showed PreToolUse deny and `updatedInput` honored on the main thread but no hook events inside child threads and role `sandbox_mode` ignored both ways; the first REAL Codex Desktop run (the pilot project, thread `01a00c1c…`, 2026-08-16 22:46) then showed native children menxia/shangshu (depth 1) and executor (depth 2, spawned by shangshu) with `agent_role` recorded, the gate denying 3 main-thread calls AND one inside the shangshu child ("shangshu 不改动工作区") — so in the Desktop app hooks DO fire in children with `agent_type`; the exec-mode gap is exec-only. Follow-up landing the same day: every seat spawn is retitled `席位｜任务号` (role + task, nothing longer, ZX ruling) via `updatedInput` (Codex `task_name`, Claude `description`) and 中书's first reply must declare AgentOS + 中书 (Stop blocks once) — the the pilot project recurring desktop-visibility error's Level 2 landing.

### 2026-08-14 — Codex seat visibility is mandatory, never opt-in (owner directive)
...
### 2026-08-14 — Engineering sources extracted for real; three rulings on what enters (owner directive)
...
### 2026-08-14 — The delivery method gets a skill AND enters the zhongshu birth-load list (owner directive)
...
### 2026-08-13 — Skills stay separate; the cognitive process composes as one workflow; a method is a paper-grade Method (owner directive)
...
### 2026-08-13 — A role's constitutive methods are must-load equipment, never on-demand references (owner directive)
...
### 2026-08-13 — One resident rule body; documents are prompts, machine rules, or memory (owner directive)
...
### 2026-08-12 — Error learning is a separate background censor; the system grinds sharper through real work (owner directive)
...
### 2026-08-12 — The working tree keeps only load-bearing content; fossils are archived, never hoarded (owner directive)
...
### 2026-08-12 — Capability profiles per seat; task-size judgment banned; Shangshu is the chain's center (owner directive)
...
### 2026-08-12 — The constitution is the kernel's first document (owner directive)
...
### 2026-08-12 — Judgment stays in thin documents; judgment-free actions sink into deterministic structure (owner directive)
...
### 2026-08-12 — Execution-chain activation quarantined; work restarts from the green 2.0 release (owner directive)
...
- 决策①：只手动触发——the owner 明确说（/fusion、"跑 Fusion"）才跑；禁止自动触发，也禁止建议式触发。理由：面板成本约为单答 N 倍，触发权属钱路，归 ZX。
...
- 决策:①所有门每轮必须处置留痕——审计条目扩为六行,gates 行逐门表态(intent= 必有,处置≥3),静默跳门变成白纸黑字可追责;②意图门带内容锚点——intent 行必须含用户原话逐字引用,Stop 钩子做子串机械验真(通知/命令轮豁免);③长交付轮(≥1200字)必须先过零上下文复述测试,复述不出重写;④"the owner"开头是 ZX 的上下文丢失金丝雀,永不机械检查/补全——约束进钩子,诊断信号永不机械化(Fable 主脑报警项 ZX 裁定不装)。
